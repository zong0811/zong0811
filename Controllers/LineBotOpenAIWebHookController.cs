    ...
    try {
        ...
        var tools = new object[] {
            new { function_declarations = new object[] { 
                ...
                new { name = "calendar_list", description = "查詢接下來一週行程" },
                new { name = "calendar_add", description = "新增日曆行程", parameters = new { type = "object", properties = new { summary = new { type = "string" }, startTime = new { type = "string", description = "格式: yyyy-MM-ddTHH:mm:ss+08:00" }, endTime = new { type = "string", description = "格式: yyyy-MM-ddTHH:mm:ss+08:00" } }, required = new[] { "summary", "startTime", "endTime" } } },
                new { name = "gmail_send", description = "寄電子郵件", parameters = new { type = "object", properties = new { recipient = new { type = "string" }, subject = new { type = "string" }, body = new { type = "string" } }, required = new[] { "recipient", "subject", "body" } } },
                new { name = "add_lesson_note", description = "記錄教學發現、筆記或整理好的政策/新聞。涉及歸檔必用。", parameters = new { type = "object", properties = new { category = new { type = "string" }, title = new { type = "string" }, content = new { type = "string" } }, required = new[] { "category", "title", "content" } } }
            } }
        };
        ...
        foreach (var p in parts) {
            if (p.functionCall != null) {
                ...
                if (funcName == "add_lesson_note") {
                    latestTitle = (string)args["title"];
                    latestContent = (string)args["content"];
                }
        ...
        if (hasFunctionCall) {
            ...
            // 這裡不放 historyList，只放當次的對話，強迫 AI 聚焦！
            ...
            // 加入明確的結束指令
            finalContents.Add(new { role = "user", parts = new[] { new { text = "請針對剛才執行的任務內容，依照格式提供詳細的總結回報，不要提及以前的對話。" } } });
            ...
            // 🌟 優化點 3：強化 Fallback (如果 AI 斷片，我們自己動手把剛才存的內容吐出來)
            if (string.IsNullOrEmpty(aiText)) {
                StringBuilder sb = new StringBuilder();
                if (!string.IsNullOrEmpty(latestContent)) {
                    sb.AppendLine($"### {latestTitle}");
                    sb.AppendLine(latestContent); 
                    sb.AppendLine("\n---");
                }
        ...
        return (string?)parts[0]?.text ?? "導師正在處理中...";
    }
