        ...
        public static async Task<(string text, int tokens)> GetResponseAsync(string userId, string userQuery)
        {
            ...
            var requestBody = new {
                ...
                systemInstruction = new { 
                    parts = new[] { new { 
                        text = $"你是一位資深的教育人員。現在是台灣時間 {time}。請用溫柔而堅定的語氣協助使用者。" +
                               "【格式規範】：回覆結束後必須換行輸入 '|' 符號，隨後提供 3 個追問按鈕標題（每個 10 字內），用逗號隔開。" +
                               "範例：這是我的建議... \n| 更短版本, 適合的小故事, 居家活動建議"
                    } } 
                },
