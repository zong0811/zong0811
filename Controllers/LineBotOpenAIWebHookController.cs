            ...
            try {
                ...
                var tools = new object[] {
                    new { function_declarations = new object[] { 
                        ...
                        new { name = "add_lesson_note", description = "記錄教學發現、教案筆記或整理好的內容。涉及歸檔必用。", parameters = new { type = "object", properties = new { category = new { type = "string" }, title = new { type = "string" }, content = new { type = "string" } }, required = new[] { "category", "title", "content" } } }
                    } }
                };
        ...
        string systemPrompt = $"你是一位具備 30 年資歷的專業教育人員，也是使用者的專屬助手。現在時間 {currentTimeInfo}。\n" +
                ...
                foreach (var p in parts) {
                    if (p.functionCall != null) {
                        ...
                        if (funcName == "add_lesson_note") { latestTitle = (string)args["title"]; latestContent = (string)args["content"]; }
                ...
                if (hasFunctionCall) {
                    ...
                    if (string.IsNullOrEmpty(aiText)) {
                        ...
                        if (!string.IsNullOrEmpty(latestContent)) { sb.AppendLine($"### {latestTitle}"); sb.AppendLine(latestContent); sb.AppendLine("\n---"); }
                        sb.AppendLine("宗志老師，任務皆已處理完成：");
                        foreach(dynamic mp in modelParts) sb.AppendLine($"✅ 已執行：{mp.functionCall.name}");
                        return sb.ToString();
                    }
                    return aiText;
                }
